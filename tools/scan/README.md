...
- **Active (opt-in `--active`)** — safe, benign probes for **SQL injection** (error-based), **open redirect** (a harmless external target reflected into `Location`), **reflected XSS** (an inert marker that comes back unescaped), and **server-side template injection** (an arithmetic marker the engine evaluates, e.g. `{{7*7}}` returning `49`). No data is touched and no credentials are tried. Each active finding is **proven, not guessed**: it carries a `PROVEN` badge plus a proof-of-concept — the exact request and the evidence that confirmed it — printed under the finding and included in JSON/SARIF output. Findings the probe couldn't demonstrate aren't reported as confirmed.
